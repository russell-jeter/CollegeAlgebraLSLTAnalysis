...
def get_distractor_counts_frame(dict_of_dfs=None):
    ...
    distractor_selection_counts = student_responses_with_details.groupby(by=['question_id', 'selected_option']).size()
    distractor_selection_counts = distractor_selection_counts.to_frame(name="count").reset_index()
    ...
    total_counts = distractor_selection_counts.groupby("question_id")["count"].transform("sum")
...
def get_percent_of_distractors_by_form(distractor_selection_counts=None):
    ...
    for exam_id in exam_ids:
        ...
        exam_distractors_chosen_dict["rarely_chosen"] = len(exam_selection_frame[
            (exam_selection_frame["percent"] > 0) & (exam_selection_frame["percent"] <= 0.05)
        ])
    ...
    exam_distractors_chosen_frame["never_chosen_percent"] = exam_distractors_chosen_frame["never_chosen"] / total
    exam_distractors_chosen_frame["rarely_chosen_percent"] = exam_distractors_chosen_frame["rarely_chosen"] / total
    exam_distractors_chosen_frame["sometimes_chosen_percent"] = exam_distractors_chosen_frame["sometimes_chosen"] / total
...
def get_effective_distractors_by_form(distractor_selection_counts=None):
    ...
    distractors_only = distractor_selection_counts[distractor_selection_counts["is_distractor"] == 1]
    
    # Group by question_id to count effective ones
    # We sum 'is_effective' (boolean becomes int)
    effective_counts_per_question = distractors_only.groupby("question_id")["is_effective"].sum().reset_index()
    effective_counts_per_question.rename(columns={"is_effective": "num_effective"}, inplace=True)
    ...
    effective_counts_per_question["exam_id"] = effective_counts_per_question["question_id"].map(exam_id_map)
...
def get_effective_distractors_per_question(distractor_counts_frame=None):
    ...
    # Count per question
    counts = effective_only.groupby("question_id").size().reset_index(name="effective_distractors")
    ...
    effective_distractors_frame = pd.merge(full_df, counts, on="question_id", how="left").fillna(0)
    effective_distractors_frame["effective_distractors"] = effective_distractors_frame["effective_distractors"].astype(int)
...
def add_effective_distractors_subplot(distractor_counts_dict, axis, exam_keys, title):
    ...
    for key in exam_keys:
        ...
        exam_effective_distractor_percents = distractor_counts_dict[key][:4] # Take first 4 bins
        ...
        for j in range(len(exam_effective_distractor_percents)):
            ...
            if exam_effective_distractor_percents[j] >= 20:
                color = text_color[bar_count % len(text_color)]
                axis.text(labels[j], y_position, f"{exam_effective_distractor_percents[j]:.2f}", 
                          color=color, ha='center', va='bottom', fontsize=10)
        bar_count += 1
