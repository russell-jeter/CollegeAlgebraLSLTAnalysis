...
def load_database_to_dict_of_dfs(df_filename=None):
    ...
    if ".xlsx" not in df_filename:
        raise ValueError(f"The filename {df_filename} was not valid. Please input an xlsx file.")
...
def get_completed_answer_choices(dict_of_dfs=None):
    ...
    if "A" in pd.unique(completed_answer_choices["option_id"]):
         ...
         completed_answer_choices["option_id"] = [ord(letter) - 64 for letter in completed_answer_choices["option_id"]]
...
def get_student_responses(dict_of_dfs=None):
    ...
    student_responses = student_responses[
        student_responses["question_id"].isin(pd.unique(completed_answer_choices["question_id"]))
    ]
...
def get_questions_per_exam(completed_answer_choices=None):
    ...
    question_counts = question_frame.groupby(by=["exam_id"])["question_id"].count().reset_index()
...
def get_exam_scores(student_responses_with_details=None):
    ...
    grouped_scores = student_score_frame.groupby(["student_id", "exam_id"])["question_score"].sum().reset_index()
    ...
    exam_scores["exam_score_percent"] = exam_scores["exam_score"] / exam_scores["number_of_questions"]
