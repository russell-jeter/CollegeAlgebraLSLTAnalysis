...
def get_kr_20_frame(student_responses_with_details=None, exam_scores=None):
    ...
    question_stats = student_score_frame.groupby(["exam_id", "question_id"])["question_score"].agg(['sum', 'count']).reset_index()
    ...
    kr_data = pd.DataFrame({
        "K": k_per_exam,
        "sum_pq": sum_pq_per_exam,
        "variance": variance_per_exam
    })
    ...
    kr_data["r"] = (kr_data["K"] / (kr_data["K"] - 1)) * (1 - (kr_data["sum_pq"] / kr_data["variance"]))
