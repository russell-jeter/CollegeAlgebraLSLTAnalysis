...
def get_item_difficulty_frame():
    ...
    item_difficulty_frame = distractor_selection_counts[
        distractor_selection_counts["is_distractor"] == 0
    ].copy()

    item_difficulty_frame = item_difficulty_frame[
        ["question_id", "exam_id", "count", "percent"]
    ]
    item_difficulty_frame = item_difficulty_frame.rename(
        columns={"percent": "item_difficulty"}
    )
...
def get_student_score_frame(student_responses_with_details=None):
    ...
    student_score_frame = pd.merge(
        student_score_frame, exam_totals, on=["student_id", "exam_id"], how="left"
    )

    # Calculate exam score MINUS the current question score (part-whole correction often used in PBC)
    student_score_frame["exam_score"] = (
        student_score_frame["total_exam_score"] - student_score_frame["question_score"]
    )
...
def get_point_biserial_coefficient_frame(student_score_frame=None):
    ...
    point_biserial_correlation_frame = student_score_frame.groupby("question_id")[
        ["question_score", "exam_score"]
    ].corr()
    ...
    point_biserial_correlation_frame = point_biserial_correlation_frame.rename(
        columns={"question_score": "pbc"}
    )
    point_biserial_correlation_frame = point_biserial_correlation_frame[
        ["question_id", "pbc"]
    ]
    point_biserial_correlation_frame["exam_id"] = point_biserial_correlation_frame[
        "question_id"
    ].str[0:2]

    # Calculate p_value (mean score / difficulty) for each question
    question_stats = (
        student_score_frame.groupby("question_id")["question_score"]
        .mean()
        .reset_index()
    )
    ...
    point_biserial_correlation_frame = pd.merge(
        point_biserial_correlation_frame, question_stats, on="question_id", how="left"
    )
    ...
    exam_stats = (
        student_score_frame.groupby("exam_id")
        .agg(k=("question_id", "nunique"), n=("student_id", "nunique"))
        .reset_index()
    )
    ...
    exam_stats.loc[mask_C, "poor_threshold"] = (
        exam_stats.loc[mask_C, "term_k"] - exam_stats.loc[mask_C, "term_n_half"]
    )
...
def show_item_difficulty_statistics(item_difficulty_frame=None):
    ...
    mean_series = item_difficulty_frame.groupby(by=["exam_id"])[
        "item_difficulty"
    ].mean()
    summary_frame = mean_series.to_frame()
    summary_frame = summary_frame.rename(columns={"item_difficulty": "mean"})
    summary_frame["median"] = item_difficulty_frame.groupby(by=["exam_id"])[
        "item_difficulty"
    ].median()
    summary_frame["std"] = item_difficulty_frame.groupby(by=["exam_id"])[
        "item_difficulty"
    ].std()
    summary_frame["skewness"] = item_difficulty_frame.groupby(by=["exam_id"])[
        "item_difficulty"
    ].skew()
    summary_frame["kurtosis"] = item_difficulty_frame.groupby(by=["exam_id"])[
        "item_difficulty"
...
def add_item_difficulty_subplot(item_difficulty_frame, axis, bins, exam_keys, title):
    ...
    for key in exam_keys:
        data_list.append(
            item_difficulty_frame[item_difficulty_frame["exam_id"].isin([key])][
                "item_difficulty"
            ].values
        )
...
def save_item_difficulty_distributions(item_difficulty_frame=None, filename=None):
    ...
    add_item_difficulty_subplot(
        item_difficulty_frame, ax0, bins, ["1A", "1B"], "Exam 1"
    )
    add_item_difficulty_subplot(
        item_difficulty_frame, ax1, bins, ["2A", "2B", "2C"], "Exam 2"
    )
    add_item_difficulty_subplot(
        item_difficulty_frame, ax2, bins, ["3A", "3B", "3C"], "Exam 3"
    )
    add_item_difficulty_subplot(
        item_difficulty_frame, ax3, bins, ["4A", "4B", "4C"], "Exam 4"
    )
...
def add_pbc_subplot_with_dynamic_threshold(
    point_biserial_correlation_frame, axis, exam_keys, title
):
    ...
    for key in exam_keys:
        exam_bar_data = []
        exam_pbc_values = point_biserial_correlation_frame[
            point_biserial_correlation_frame["exam_id"].isin([key])
        ]
        ...
        # Count categories
        count_poor = len(
            exam_pbc_values[exam_pbc_values["pbc"] <= exam_pbc_values["poor_threshold"]]
        )
        count_good = len(
            exam_pbc_values[exam_pbc_values["pbc"] >= exam_pbc_values["good_threshold"]]
        )
...
def save_pbc_distribution_plots_with_thresholds(
    point_biserial_correlation_frame=None, filename=None
):
    ...
    add_pbc_subplot_with_dynamic_threshold(
        point_biserial_correlation_frame, ax0, ["1A", "1B"], "Exam 1"
    )
    add_pbc_subplot_with_dynamic_threshold(
        point_biserial_correlation_frame, ax1, ["2A", "2B", "2C"], "Exam 2"
    )
    add_pbc_subplot_with_dynamic_threshold(
        point_biserial_correlation_frame, ax2, ["3A", "3B", "3C"], "Exam 3"
    )
    add_pbc_subplot_with_dynamic_threshold(
        point_biserial_correlation_frame, ax3, ["4A", "4B", "4C"], "Exam 4"
    )
...
def add_pbc_subplot(point_biserial_correlation_frame, axis, bins, exam_keys, title):
    data_list = []
    for key in exam_keys:
        val = point_biserial_correlation_frame[
            point_biserial_correlation_frame["exam_id"].isin([key])
        ]["pbc"].values
...
def save_pbc_distribution_plots(point_biserial_correlation_frame=None, filename=None):
    ...
    bins = [-0.5, -0.25, 0, 0.25, 0.5, 0.75, 1]

    fig, ((ax0, ax1), (ax2, ax3)) = plt.subplots(nrows=2, ncols=2, figsize=(10, 6))

    add_pbc_subplot(point_biserial_correlation_frame, ax0, bins, ["1A", "1B"], "Exam 1")
    add_pbc_subplot(
        point_biserial_correlation_frame, ax1, bins, ["2A", "2B", "2C"], "Exam 2"
    )
    add_pbc_subplot(
        point_biserial_correlation_frame, ax2, bins, ["3A", "3B", "3C"], "Exam 3"
    )
    add_pbc_subplot(
        point_biserial_correlation_frame, ax3, bins, ["4A", "4B", "4C"], "Exam 4"
    )
    ...
    if use_arbitrary_binning:
        ...
        for exam_id in exam_ids:
            ...
            pbc_bin_assignment = np.digitize(
                point_biserial_correlation_frame[
                    point_biserial_correlation_frame["exam_id"].isin([exam_id])
                ]["pbc"],
                pbc_bins,
                right=False,
            )
