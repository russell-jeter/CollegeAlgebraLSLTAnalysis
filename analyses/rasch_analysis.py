...
def questions_to_columns(df):
    ...
    restructured_df = df.pivot(index='student_id', columns='question_id', values='selected_option')
    
    # Drops bad questions
    bad_question = ['4A01', '4B01', '4C01'] # Exam 4 question 1 is "what version exam do you have"
    restructured_df = restructured_df.drop(columns=[q for q in bad_question if q in restructured_df.columns])
...
def create_num_ver_key_dict(num_and_ver, key_df):
    ...
    option_ids = exam_key_df['option_id'].replace({'A': '1', 'B': '2', 'C': '3', 'D': '4', 'E': '5'})
...
def remove_issue_scores(df):
    ...
    students_to_drop = student_scores[(student_scores == max_score) | (student_scores == 0)].index
    ...
    questions_to_drop = question_means[(question_means == 0) | (question_means == 1)].index
...
def create_true_false_for_all_exams(full_df, key_df, all_exam_numbers_and_forms):
    ...
    for exam_num_and_form in all_exam_numbers_and_forms:
        ...
        list_of_tf_dfs.append({
            'exam_num_and_form': exam_num_and_form, 
            'true_false_df': temp_exam_tf_df
        })
...
def approximate_ability_and_difficulty(df, answer_choices_df, questions_df=None):
    ...
    if not use_coeffs:
        ...
        merged = pd.merge(mapping_df, questions_df[['question_id', 'difficulty_coeff']], on='question_id', how='left')
...
def build_rasch_model(base_df, answer_choices_df, model_parameters, questions_df=None):
    ...
    variable_estimates_dict = approximate_ability_and_difficulty(base_df, answer_choices_df, questions_df)
    ...
    while abs(sum_sqr_res_current - sum_sqr_res_previous) > 0.001 and iteration_num < max_iterations:
        if iteration_num > 0:
            variable_estimates_dict = iterate_variable_estimates(variable_estimates_dict, est_var_ex_vals_df, residuals_df)
        
        expected_values_df = calc_expected_values(variable_estimates_dict, model_parameters)
...
def build_rasch_dfs(list_of_rasch_dicts):
    ...
    for rasch_dict in list_of_rasch_dicts:
        # 1. Process Students
        student_stats_keys = [
            'var_estimates_students_1PL', 'outfit_students_1PL', 'infit_students_1PL', 
        ...
        # SE = sqrt(2/N_items)
        temp_standard_error = math.sqrt(2 / len(temp_student_df)) if len(temp_student_df) > 0 else 0
        ...
        for model_parameters in [1, 3]:
            ...
            def categorize(val, se):
                lower = 1 - 2*se
                upper = 1 + 2*se
                if val <= lower: return 1 # Low fit means overly predictable
            ...
            for measure in ['outfit', 'infit']:
                ...
                if col in temp_student_df.columns:
                    ...
                    temp_student_df[f'is_good_{measure}{suffix}'] = (val <= (1 - 2*temp_standard_error)).astype(int)
                    temp_student_df[f'is_acceptable_{measure}{suffix}'] = ((val > (1 - 2*temp_standard_error)) & (val < (1 + 2*temp_standard_error))).astype(int)
                    temp_student_df[f'is_poor_{measure}{suffix}'] = (val >= (1 + 2*temp_standard_error)).astype(int)

        list_of_student_dfs.append(temp_student_df)

        # 2. Process Items
        item_stats_keys = [
            'var_estimates_items_1PL', 'outfit_items_1PL', 'infit_items_1PL', 
        ...
        for model_parameters in [1, 3]:
            ...
            for measure in ['outfit', 'infit']:
                ...
                if col in temp_item_df.columns:
                    ...
                    temp_item_df[f'is_good_{measure}{suffix}'] = (val <= (1 - 2*temp_standard_error)).astype(int)
                    temp_item_df[f'is_acceptable_{measure}{suffix}'] = ((val > (1 - 2*temp_standard_error)) & (val < (1 + 2*temp_standard_error))).astype(int)
                    temp_item_df[f'is_poor_{measure}{suffix}'] = (val >= (1 + 2*temp_standard_error)).astype(int)
        
        list_of_item_dfs.append(temp_item_df)

    rasch_students_df = pd.concat(list_of_student_dfs) if list_of_student_dfs else pd.DataFrame()
...
def get_rasch_students_and_items_frames_as_dict():
    ...
    list_of_tf_dfs = create_true_false_for_all_exams(raw_df, key_df_correct, all_exam_numbers_and_forms)
    ...
    for exam_dict in list_of_tf_dfs:
        ...
        # 1PL Model
        rasch_dict_1PL = build_rasch_model(no_error_exam_df.copy(), options_df, 1, questions_df)
        
        # 3PL Model
        rasch_dict_3PL = build_rasch_model(no_error_exam_df.copy(), options_df, 3, questions_df)
