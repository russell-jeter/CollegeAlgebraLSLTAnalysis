        ...
        if get_form_code(text_array):
            ...
            for i in range(first_student_index, len(text_array) - 1):
                ...
                if is_student_row(text_row):
                    ...
                    student_dict = convert_student_row_to_dict(
                        student_row=text_row, question_array=question_array
                    )
...
student_frame = (
    student_frame.groupby("student_id")
    .agg(create_aggregate_function_dict(student_frame.columns))
    .reset_index()
)
